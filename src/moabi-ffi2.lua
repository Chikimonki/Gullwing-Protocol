...
function M.extract_ml_features(filepath)
    ...
    if dep_features then
        ...
        features.has_libselinux  = dep_features.has_libselinux  or 0.0
    end
...
function M.extract_elf_features(filepath)
    local zero = {
        ...
        has_libruby = 0,
        has_libselinux = 0, has_libcap = 0, has_pthread = 0,
        has_libdl = 0, has_libm = 0, has_librt = 0,
    }
                ...
                if sname == ".dynstr" then
                    ...
                    if ds_off > 0 and ds_size > 0 then
                        ...
                        for _, idx in ipairs(needed_names) do
                            ...
                            if lib_name:find("selinux")  then result.has_libselinux = 1 end
                            if lib_name:find("cap")      then result.has_libcap    = 1 end
                            if lib_name:find("pthread")  then result.has_pthread  = 1 end
                            if lib_name:find("dl")       then result.has_libdl    = 1 end
                            if lib_name:find("libm")     then result.has_libm     = 1 end
                            if lib_name:find("rt")       then result.has_librt    = 1 end
                        end
                    end
                    break
                end
...
function M.extract_dependency_features(filepath)
    ...
    local function has(s) return data:find(s, 1, true) and 1 or 0 end
    ...
    local deps = {
        ...
        has_ncurses = (has("libncurses")==1 or has("libtinfo")==1) and 1 or 0,
    ...
    local seen = {}
    for lib in data:gmatch("lib[%w%._%+%-]+%.so[%w%.%+%-]*") do
        if not seen[lib] then
            seen[lib] = true
            deps.libs[#deps.libs+1] = lib
        end
    end
    deps.dependency_count = #deps.libs
    return deps
end
