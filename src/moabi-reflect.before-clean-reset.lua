...
local function main()
    ...
    local model,static_only,json_out,deep_memory,containment = DEFAULT_MODEL,false,false,false,false
    ...
    if not static_only then
        ...
        ev.set_memory(
            ...
            {
                profiled = false,
                status = "analyzed", profiled = true
                status = "clean", profiled = true, unpack_detected = false
                status = "skipped",
                status = "error", profiled = false
                reason = "disabled (--static-only)",
            },
            "moabi-memory",
            0
        )
    ...
    -- Optional v2 differential analysis.
    if deep_memory and not static_only then
        local diff_start = now_ms()
        local diff_result = diff_mod.compare(target)

        ev.set_memory_differential(
            e,
            diff_mod.evidence_fragment(diff_result),
            "moabi-diff",
            now_ms() - diff_start
        )
    else
        ev.set_memory_differential(
            e,
            {
                profiled = false,
                status = "skipped",
                reason = "standalone differential disabled",
            },
            "moabi-diff",
            0
        )
